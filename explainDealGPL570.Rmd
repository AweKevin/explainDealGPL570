...
head(tars)
## 原始文件在上传的过程中会遇到很多很多的问题，所以有些问题并不是我们的问题，加上tryCatch，这样for循环遇到bug不会终止，并把bug写进AProblem.txt里面
for (i in tars) {
    tryCatch({
        ...
        write.table(data.frame(i, err %>% as.character()), "AProblem.txt", append = T, col.names = F, 
